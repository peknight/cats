...
class EvalFlatSpec extends AnyFlatSpec:
  "Eval" should "pass" in {
    val x =
      println("Computing X")
      math.random()
    ...
    assert(now.value === now.value)
    println(always.value)
    println(always.value)
    assert(later.value === later.value)
    ...
    assert(foldRightWithEval((1 to 100000).toList, 0L)(_ + _) === 5000050000L)
  }
end EvalFlatSpec
