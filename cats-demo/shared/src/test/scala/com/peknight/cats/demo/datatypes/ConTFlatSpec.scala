...
import com.peknight.cats.demo.datatypes.UserUpdateResult.{Succeeded, Failed}

class ConTFlatSpec extends AnyFlatSpec:
  private def updateUser(persistToDatabase: User => Eval[UserUpdateResult])(existingUser: User, newName: String,
                                                                            newAge: Int): Eval[UserUpdateResult] =
  ...
  private def updateUserCont(existingUser: User, newName: String, newAge: Int): ContT[Eval, UserUpdateResult, User] =
  ...
  "ConT" should "pass" in {
    ...
    val eval3 = chainOfContinuations.run { finalResult =>
      Eval.later {
        println("Finished!")
        finalResult
      }
